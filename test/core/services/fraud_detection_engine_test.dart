...
class _StubRule implements FraudRule {
  ...
  /// Simulates a gated scan (mirror never pulled, empty judgeable window):
  /// the rule returns null — no verdict — instead of "clean".
...
/// Emits noisy evidence but a stable explicit confirmation signature —
/// the shape AUDIT_TAMPER has (moving head seqs inside the evidence).
class _SignedStubRule implements FraudRule {
  int _sweep = 0;
  ...
  Future<List<FraudFlagDraft>?> evaluate(FraudScanContext ctx) async {
    ...
    return [
      FraudFlagDraft(
        ...
        title: 'stub',
        description: 'stub',
        dedupeKey: '$code|incident-1',
        detectedAt: ctx.now,
        evidence: [
          {'fact': 'noise', 'value': 'sweep-$_sweep'},
        ],
        confirmationSignature: 'stable-sig',
      ),
    ];
  }
}
...
void main() {
  ...
  group('confirmation pipeline', () {
    ...
    test('an INCONCLUSIVE scan (null) leaves candidates untouched', () async {
      await seedBusiness();
      final rule = _StubRule('TIME_REVERSAL');
      final engine = buildEngine([rule]);
      ...
      // Next sweep the rule is gated (e.g. transient watermark error) —
      // "couldn't judge" must not be mistaken for "resolved".
      rule.inconclusive = true;
      await engine.runSweep();

      final candidate =
          await candidatesDao.getByKey('biz1', 'TIME_REVERSAL|incident-1');
      expect(candidate, isNotNull);
      expect(candidate!.seenCount, 1, reason: 'no sighting was recorded');
    });

    test('an incremental run never prunes candidates it did not re-see',
        () async {
      await seedBusiness();
      final rule = _StubRule('TIME_REVERSAL');
      final engine = buildEngine([rule]);
      ...
      // Incremental windows don't cover old incidents — not seeing one
      // there is not resolution.
      rule.emit = false;
      await engine.runIncremental(const {'TIME_REVERSAL'});
      expect(
        await candidatesDao.getByKey('biz1', 'TIME_REVERSAL|incident-1'),
        isNotNull,
      );

      // A FULL sweep that doesn't re-see it IS resolution.
      await engine.runSweep();
      expect(
        await candidatesDao.getByKey('biz1', 'TIME_REVERSAL|incident-1'),
        isNull,
    ...
    test('an explicit confirmationSignature overrides the evidence hash',
        () async {
      await seedBusiness();
      // Same incident, evidence noise differs, stable explicit signature —
      // the second sighting (40 min later per the pre-seeded candidate) must
      // still promote.
      await candidatesDao.insertCandidate(
        businessId: 'biz1',
        dedupeKey: 'TIME_REVERSAL|incident-1',
      ...
      final rule = _SignedStubRule();
      await buildEngine([rule]).runSweep();
