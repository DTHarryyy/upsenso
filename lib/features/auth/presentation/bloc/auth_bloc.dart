  ...
  Future<void> _onStarted(AuthStarted event, Emitter<AuthState> emit) async {
    ...
    if (user != null) {
      if (_hasCompleteContext(user)) {
        ...
        debugPrint(
          'AuthBloc: Context incomplete — fetching before emitting to avoid false redirect',
        );
        try {
          final resolved = await _getUserContextWithRetry(user.id);
          ...
          debugPrint('AuthBloc: Error fetching context on start: $e');
          emit(AuthAuthenticated(user));
          _backgroundSync(user);
        }
      }
