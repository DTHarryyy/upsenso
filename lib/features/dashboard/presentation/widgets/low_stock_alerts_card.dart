...
class LowStockAlertsCard extends StatelessWidget {
  ...
  final ValueChanged<LowStockItem> onItemTap;
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          Row(
            children: [
              const Icon(IconlyBold.danger, size: 20, color: Color(0xFFF59E0B)),
          ...
          if (items.isEmpty)
            ...
            ...items
                .take(5)
                .map(
                  (item) => _StockRow(item: item, onTap: () => onItemTap(item)),
                ),
...
class _StockRow extends StatelessWidget {
  ...
  _Severity _severity(int stock, int? threshold) {
    if (stock <= 0) {
      return const _Severity(label: 'OUT OF STOCK', color: Color(0xFFDC2626));
    }
    if (threshold != null && stock <= threshold ~/ 2) {
      return const _Severity(label: 'CRITICAL', color: Color(0xFFEF4444));
    }
    return const _Severity(label: 'LOW', color: Color(0xFFF59E0B));
  }
}
